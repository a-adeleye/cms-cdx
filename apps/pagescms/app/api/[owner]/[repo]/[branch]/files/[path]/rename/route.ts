...
import { getFileExtension, isPathWithin, normalizePath } from "@/lib/utils/file";
  ...
  try {
    ...
    switch (data.type) {
      case "content":
        ...
        if (!isPathWithin(normalizedNewPath, schema.path)) throw new Error(`Invalid path "${data.newPath}" for ${data.type} "${data.name}".`);
      ...
      case "media":
        ...
        if (!isPathWithin(normalizedPath, schema.input)) throw new Error(`Invalid path "${params.path}" for media.`);
        if (!isPathWithin(normalizedNewPath, schema.input)) throw new Error(`Invalid path "${data.newPath}" for media.`);
...
const githubRenameFile = async (
  ...
  if (isLocalDevelopmentMode()) {
    ...
    const store = await getLocalRepositoryStore();
    const current = await store.read(path);
    const result = await store.rename(path, newPath, current.sha);
    return { sha: result.commitSha, path, newPath };
  }
