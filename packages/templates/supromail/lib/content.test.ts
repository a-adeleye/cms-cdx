...
test('parseArticleMarkdown reads a fenced block as verbatim code', () => {
  const parsed = parseArticleMarkdown('Run this:\n\n```bash\ncurl -X POST /v1/domains\n**not bold**\n```\n\nDone.');

  assert.deepEqual(parsed.blocks.map((block) => block.kind), ['paragraph', 'codeBlock', 'paragraph']);
  const code = parsed.blocks[1];
  assert.equal(code.kind === 'codeBlock' ? code.value : '', 'curl -X POST /v1/domains\n**not bold**');
});

test('parseArticleMarkdown closes an unterminated fence instead of falling back to prose', () => {
  const parsed = parseArticleMarkdown('```\nkey: value\n');

  assert.deepEqual(parsed.blocks.map((block) => block.kind), ['codeBlock']);
  assert.equal(parsed.blocks[0].kind === 'codeBlock' ? parsed.blocks[0].value : '', 'key: value');
});
...
test('normalizeBlogPath confines navigation to a root-relative path', () => {
  assert.equal(normalizeBlogPath('/resources/blog/'), '/resources/blog');
  assert.equal(normalizeBlogPath('javascript:alert(1)'), '/blog');
  assert.equal(normalizeBlogPath('//example.com/blog'), '/blog');
  assert.equal(normalizeBlogPath('/'), '/blog');
});
...
test('adaptArticle derives neutral metadata and reading time for valid CMS content', () => {
  ...
  assert.equal(article.publishedLabel, '12 Jun 2026');
  assert.equal(article.publishedLongLabel, '12 June 2026');
  assert.equal(article.publishedIso, '2026-06-12');
  assert.equal(article.author, 'Supromail team');
  assert.equal(article.authorInitials, 'ST');
  assert.equal(article.category, 'Messaging');
  assert.equal(article.categorySlug, 'messaging');
});

test('adaptArticle leaves the machine-readable date empty when the CMS date is not a plain day', () => {
  const article = adaptArticle({ title: 'Timestamped', slug: 'timestamped', publishedAt: 'not-a-date' }, 0);

  assert.equal(article.publishedLabel, '');
  assert.equal(article.publishedIso, '');
});

test('deriveInitials builds an avatar label from any author name', () => {
  assert.equal(deriveInitials('Ada Ajayi'), 'AA');
  assert.equal(deriveInitials('  nkem   eze  okafor '), 'NE');
  assert.equal(deriveInitials(''), 'SM');
});

test('collectCategories lists each category once in publication order', () => {
  const articles = [
    adaptArticle({ title: 'One', slug: 'one', categoryName: 'Deliverability' }, 0),
    adaptArticle({ title: 'Two', slug: 'two', categoryName: 'Engineering' }, 1),
    adaptArticle({ title: 'Three', slug: 'three', categoryName: 'Deliverability' }, 2),
  ];

  assert.deepEqual(collectCategories(articles), [
