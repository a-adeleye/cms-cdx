...
export async function GET(
  ...
  try {
    ...
    if (normalizedPath === ".pages.yml") {
      if (!canManageConfiguration(user)) throw createHttpError("You cannot access settings.", 403);
    }
    ...
    if (name) {
      ...
      if (!isPathWithin(normalizedPath, schema.path)) throw createHttpError(`Invalid path "${params.path}" for ${schema.type} "${name}".`, 400);
    ...
    if (isLocalDevelopmentMode()) {
      const file = await (await getLocalRepositoryStore()).read(normalizedPath);
      response = { data: { type: "file", name: file.name, path: file.path, sha: file.sha, content: file.bytes.toString("base64") } };
    } else {
      const octokit = createOctokitInstance(token);
      try {
        response = await octokit.rest.repos.getContent({ owner: params.owner, repo: params.repo, path: normalizedPath, ref: params.branch });
      } catch (error: any) {
