...
const getRepoSnapshot = async (
  ...
  if (isLocalDevelopmentMode()) {
    const local = await assertLocalRepositoryRef(owner, repo);
    return {
      id: 0,
      owner: local.owner,
      ownerId: 0,
...
const getCollectionCache = async (
  ...
  if (isLocalDevelopmentMode()) {
    ...
    return entries.map((entry) => ({
      ...
      isNode: nodeEntryFilename ? entry.name === nodeEntryFilename : undefined,
    }));
  }
...
const getMediaCache = async (
  ...
  if (isLocalDevelopmentMode()) {
    await assertLocalRepositoryRef(owner, repo, branch);
    const entries = await (await getLocalRepositoryStore()).list(path);
    return entries.map((entry) => ({
      id: -1,
      ...entry,
      context: "media",
