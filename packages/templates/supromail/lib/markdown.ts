...
function readBlock(lines: string[], cursor: number): { block: UnparsedBlock; cursor: number } {
  ...
  if (/^```/.test(line)) return readCodeBlock(lines, cursor);
...
// An unterminated fence consumes the remaining lines rather than falling back to
// paragraph parsing, so half-written CMS content still renders as inert code.
function readCodeBlock(lines: string[], cursor: number): { block: UnparsedBlock; cursor: number } {
  const code: string[] = [];
  cursor += 1;
  ...
  return { block: { kind: 'codeBlock', value: code.join('\n') }, cursor: cursor + 1 };
}
...
function finalizeBlock(block: UnparsedBlock, headingIds: Map<string, number>): MarkdownBlock {
  ...
  if (block.kind === 'codeBlock') {
    return { kind: 'codeBlock', value: block.value };
  }
  if (block.kind === 'list') {
