...
import { configVersion, parseAndValidateConfig, parseConfig, normalizeConfig } from "@/lib/config";
...
import { getFileExtension, getFileName, isPathWithin, normalizePath, serializedTypes, getParentPath } from "@/lib/utils/file";
import { canManageConfiguration } from "@/lib/authz-shared";
...
import { assertLocalRepositoryRef } from "@/lib/local-dev/config-server";
  ...
  try {
    ...
    switch (data.type) {
      case "content":
        ...
        if (!isPathWithin(normalizedPath, schema.path)) throw new Error(`Invalid path "${params.path}" for ${data.type} "${data.name}".`);
      ...
      case "media":
        ...
        if (!isPathWithin(normalizedPath, schema.input)) throw new Error(`Invalid path "${params.path}" for media "${data.name}".`);
        ...
        break;
      case "settings":
        if (!canManageConfiguration(user)) throw createHttpError("You cannot manage settings.", 403);
        ...
        const validatedConfig = parseAndValidateConfig(data.content.body ?? "");
        if (validatedConfig.parseErrors.length > 0 || validatedConfig.validationErrors.some((error: any) => error.severity !== "warning")) {
          throw createHttpError("Configuration validation failed. Fix the highlighted errors before saving.", 400);
        }
        contentBase64 = Buffer.from(data.content.body ?? "").toString("base64");
        break;
      default:
...
const githubSaveFile = async (
  ...
  if (isLocalDevelopmentMode()) {
    await assertLocalRepositoryRef(owner, repo, branch);
    try {
      const result = await (await getLocalRepositoryStore()).write(path, Buffer.from(contentBase64, "base64"), sha);
      return { data: {
        content: { type: "file", name: result.file.name, path: result.file.path, sha: result.file.sha, size: result.file.size, download_url: result.file.downloadUrl },
        commit: { sha: result.commitSha, committer: { date: result.committedAt } },
      } };
    } catch (error: any) {
      if (error?.status === 422 && !sha && options?.onConflict !== "error") {
        ...
        for (let suffix = 1; suffix <= 3; suffix++) {
          const candidate = `${parentDir ? `${parentDir}/` : ""}${stem}-${suffix}${extension}`;
          try {
            const result = await (await getLocalRepositoryStore()).write(candidate, Buffer.from(contentBase64, "base64"));
            return { data: {
              content: { type: "file", name: result.file.name, path: result.file.path, sha: result.file.sha, size: result.file.size, download_url: result.file.downloadUrl },
              commit: { sha: result.commitSha, committer: { date: result.committedAt } },
            } };
          } catch (candidateError: any) {
            if (candidateError?.status !== 422 || suffix === 3) throw candidateError;
          }
        }
      }
      throw error;
  ...
  try {
    ...
    switch (type) {
      case "content":
        ...
        if (!isPathWithin(normalizedPath, schema.path)) throw new Error(`Invalid path "${params.path}" for ${type} "${name}".`);
      ...
      case "media":
        ...
        if (!isPathWithin(normalizedPath, schema.input)) throw new Error(`Invalid path "${params.path}" for media "${name}".`);
    ...
    const response: any = isLocalDevelopmentMode()
      ? (() => getLocalRepositoryStore().then(async (store) => {
          const result = await store.delete(normalizedPath, sha);
          return { data: { content: null, commit: { sha: result.commitSha, committer: { date: result.committedAt } } } };
        }))()
      : createOctokitInstance(token).rest.repos.deleteFile({
      owner: params.owner,
      repo: params.repo,
      branch: params.branch,
      path: normalizedPath,
    ...
    await updateFileCache(
      ...
      params.branch,
      {
        type: 'delete',
        path: normalizedPath,
        commit: resolvedResponse?.data.commit?.sha
          ? {
              sha: resolvedResponse.data.commit.sha,
              timestamp: new Date(
                resolvedResponse.data.commit.committer?.date ?? new Date().toISOString(),
              ).getTime(),
            }
          : undefined,
      }
    ...
    return Response.json({
      ...
      data: {
        sha: resolvedResponse?.data.commit.sha,
        name: resolvedResponse?.data.content?.name,
        path: resolvedResponse?.data.content?.path,
      }
    });
  } catch (error: any) {
    console.error(error);
