...
import { isLocalDevelopmentMode, LOCAL_DEVELOPMENT_TOKEN } from "@/lib/local-dev/mode";
...
const getToken = cache(async (
  ...
  if (isLocalDevelopmentMode()) {
    await assertLocalRepositoryRef(owner, repo);
    return { token: LOCAL_DEVELOPMENT_TOKEN, source: "local" as const };
  }
