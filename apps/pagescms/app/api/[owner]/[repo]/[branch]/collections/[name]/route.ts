...
import { isPathWithin, normalizePath } from "@/lib/utils/file";
...
export async function GET(
  ...
  try {
    ...
    if (!isPathWithin(normalizedPath, schema.path)) throw createHttpError(`Invalid path "${path}" for collection "${params.name}".`, 400);
